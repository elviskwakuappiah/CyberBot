...
function getJwtSecret(): string {
  ...
  if (!secret) {
    if (process.env.NODE_ENV === 'production') {
      throw new Error('FATAL: JWT_SECRET environment variable is NOT set. Production access denied.');
    }
    // Fallback for local development only
    console.warn('WARNING: JWT_SECRET not set. Using insecure development fallback.');
    return 'dev-secret-key-only-do-not-use-in-production';
  }
  return secret;
}
...
async function startServer() {
  ...
  app.post('/api/auth/2fa/verify', async (req, res) => {
    ...
    // For this implementation, we use a static code '123456' as the 2FA code
    if (code !== '123456') {
