...
import { RobotUnit, Bullet, Enemy, Upgrades } from '../types';
...
const ROBOT_STATS: Record<RobotUnit, any> = {
  ...
  [RobotUnit.TITAN]: { 
    name: "TITAN", color: '#ef4444', hpMult: 2.5, 
...
const GameView: React.FC<GameViewProps> = ({ level, upgrades, isPaused, onWin, onDie, onTitanScene }) => {
  ...
  const drawAltMode = (ctx: CanvasRenderingContext2D, unit: RobotUnit, frame: number, alpha: number, vel: {x:number}, flightActive?: boolean, isDying?: boolean, deathTimer?: number) => {
    ...
    switch (unit) {
      ...
      case RobotUnit.TITAN: 
        drawMechanicalPart(ctx, -40, -5, 80, 25, '#0a0a0a', 2); 
        drawMechanicalPart(ctx, -30, -22, 60, 22, color, 4);
  ...
  const drawUnit = (ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, unit: RobotUnit, facing: number, frame: number, isEnemy: boolean, invulnerable: boolean, transformed: boolean, alpha: number, vel: {x:number}, sizeMult = 1, isCombiner = false, type: string = 'grunt', flightActive?: boolean, isDying?: boolean, deathTimer?: number, pose: 'normal' | 'victory' | 'limb-arm' | 'limb-leg' | 'limb-torso' = 'normal', limbColors?: string[], abilityActive?: boolean) => {
    ctx.save();
    ...
    const drawAlpha = invulnerable ? (0.5 + Math.sin(frame * 0.5) * 0.3) * alpha : alpha;
    const stats = ROBOT_STATS[unit];
  ...
  const resetLevel = useCallback(() => {
    ...
    const isBossLevel = [10, 20, 30].includes(level);
    const isLevel21 = level === 21;
    const isLevel22 = level === 22;

    if (isLevel22) {
      e.enemies = []; // No enemies in level 22
      e.rescueShipPos = { x: 3000, y: GROUND_Y - 200 };
      // Create bodies for the squad members (except the one the player is currently using)
      // Actually, the user said "collect the bodies of his/her comrades' bodies in his/her squad"
      // Since the squad was destroyed, we should probably spawn bodies of all robots that were in the squad.
      // But wait, the user just selected ONE robot for the rescue mission.
      // So the bodies should be the OTHER robots that were in the squad before.
      // However, for simplicity and to make it a real mission, let's just spawn 4 bodies.
      const squadUnits = upgrades.activeSquad || [];
      e.bodies = [
        { unit: RobotUnit.SENTINEL, pos: { x: 800, y: GROUND_Y - 20 }, grabbed: false, delivered: false },
        { unit: RobotUnit.STRIKER, pos: { x: 1400, y: GROUND_Y - 20 }, grabbed: false, delivered: false },
        { unit: RobotUnit.FALCON, pos: { x: 2000, y: GROUND_Y - 20 }, grabbed: false, delivered: false },
        { unit: RobotUnit.TITAN, pos: { x: 2600, y: GROUND_Y - 20 }, grabbed: false, delivered: false },
      ];
    } else if (isLevel21) {
      e.enemies = [
        { id: 2101, pos: { x: 2500, y: GROUND_Y - 400 }, vel: { x: 0, y: 0 }, width: 300, height: 400, health: 10000, maxHealth: 10000, isDead: false, type: 'gigantic_titan', lastShot: 0, facing: -1 },
      ];
      e.shipPos = { x: 600, y: GROUND_Y - 150 };
      e.shipVel = { x: 0, y: 0 };
      e.shipHealth = 2000;
    } else if (isBossLevel) {
      const bossHP = 5000 * (1 + (level - 10) * 0.1);
      e.enemies = [
        { id: 101, pos: { x: 2500, y: GROUND_Y - 200 }, vel: { x: 0, y: 0 }, width: 120, height: 200, health: bossHP, maxHealth: bossHP, isDead: false, type: 'titan', lastShot: 0, facing: -1 },
        { id: 102, pos: { x: 3500, y: 200 }, vel: { x: 0, y: 0 }, width: 150, height: 100, health: bossHP * 0.6, maxHealth: bossHP * 0.6, isDead: false, type: 'dragon', lastShot: 0, facing: -1 },
        { id: 103, pos: { x: 4500, y: GROUND_Y - 100 }, vel: { x: 0, y: 0 }, width: 100, height: 100, health: bossHP * 0.5, maxHealth: bossHP * 0.5, isDead: false, type: 'omnidroid', lastShot: 0, facing: -1 },
      ];
    } else {
      const enemyCount = 5 + Math.floor(level / 5);
      e.enemies = Array.from({ length: enemyCount }, (_, i) => {
        let type = 'seeker';
        if (level >= 5 && i % 4 === 0) type = 'tank';
        else if (level > 15 && Math.random() < 0.3) type = 'tank';
        ...
        return { 
          id: i, 
          pos: { x: 1800 + i * 1100, y: GROUND_Y - (isTank ? 40 : 72) }, 
          vel: { x: 0, y: 0 }, 
          width: isTank ? 80 : 48, 
          height: isTank ? 40 : 72, 
          health: (isTank ? 200 : 100) * (1 + level * 0.15), 
          maxHealth: (isTank ? 200 : 100) * (1 + level * 0.15), 
          isDead: false, 
          type: type, 
          lastShot: 0, 
          facing: -1,
  ...
  const drawCityscape = (ctx: CanvasRenderingContext2D, e: any) => {
    ...
    const layers = [
      { scroll: 0.1, color: '#0c0c18', spacing: 450, hMin: 300, hMax: 600 },
      { scroll: 0.4, color: '#1a0f0a', spacing: 400, hMin: 400, hMax: 850 },
      { scroll: 0.8, color: '#050505', spacing: 350, hMin: 500, hMax: 1000 },
    ];
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (e.isCombined && leader) {
        const s = { speed: 6, jump: -22, fireRate: 4 };
        if (moveRight) { leader.vel.x = s.speed; leader.facing = 1; } else if (moveLeft) { leader.vel.x = -s.speed; leader.facing = -1; } else leader.vel.x *= 0.8;
        if (moveUp && leader.grounded) { leader.vel.y = s.jump; leader.grounded = false; }
        ...
        if (firing && e.frame - leader.lastShot > s.fireRate) {
          soundService.playShoot();
          for(let i=-2; i<=2; i++) e.bullets.push({ pos: { x: leader.pos.x + 70, y: leader.pos.y + 80 + (i*30) }, vel: { x: leader.facing * 45, y: i * 4 }, width: 50, height: 16, owner: 'player', damage: 60, color: '#facc15', health: 1, maxHealth: 1 });
      ...
      e.enemies.forEach(en => {
        ...
        if (en.type === 'gigantic_titan') {
          ...
          if (e.frame % 80 === 0) {
            ...
            for(let i=0; i<8; i++) {
              e.bullets.push({ 
                pos: { x: en.pos.x, y: en.pos.y + 50 + i*40 }, 
                vel: { x: -10, y: (i-3.5)*1.5 }, 
                width: 50, height: 25, owner: 'enemy', health: 1, maxHealth: 1, damage: 80, color: '#ff00ff' 
              });
            }
            // Homing missiles
            if (leader) {
              const dx = (e.isShipActive ? e.shipPos.x : leader.pos.x) - en.pos.x;
              const dy = (e.isShipActive ? e.shipPos.y : leader.pos.y) - en.pos.y;
              const dist = Math.sqrt(dx*dx + dy*dy);
              e.bullets.push({ 
          ...
          en.pos.x += Math.sign((leader?.pos.x || 0) - en.pos.x) * 1.2 * difficultyFactor;
          if (leader && Math.abs(en.pos.x - leader.pos.x) < 1600 && e.frame - en.lastShot > 100 / difficultyFactor) {
            e.bullets.push({ pos: { x: en.pos.x, y: en.pos.y + 100 }, vel: { x: Math.sign(leader.pos.x - en.pos.x) * 10 * difficultyFactor, y: 0 }, width: 60, height: 40, owner: 'enemy', health: 1, maxHealth: 1, damage: 100, color: '#ff0055' });
            en.lastShot = e.frame;
          }
            ...
            if (en.isTransformed) {
              ...
              if (leader && Math.abs(en.pos.x - leader.pos.x) < 1200 && e.frame - en.lastShot > 80 / difficultyFactor) {
              ...
              if (leader && Math.abs(en.pos.x - leader.pos.x) < 1400 && e.frame - en.lastShot > 100 / difficultyFactor) {
          ...
          if (leader && e.frame - en.lastShot > 60 / difficultyFactor) {
          ...
          if (leader && e.frame - en.lastShot > 40 / difficultyFactor) {
          ...
          en.pos.x += en.vel.x;
          if (leader && Math.abs(en.pos.x - leader.pos.x) < 1400 && e.frame - en.lastShot > (en.isTransformed ? 120 : 60) / difficultyFactor) {
            e.bullets.push({ pos: { x: en.pos.x, y: en.isTransformed ? en.pos.y - 15 : en.pos.y + 30 }, vel: { x: Math.sign(leader.pos.x - en.pos.x) * 12 * difficultyFactor, y: 0 }, width: 40, height: 20, owner: 'enemy', health: 1, maxHealth: 1, damage: 50, color: '#550000' });
            en.lastShot = e.frame;
          }
        }
    ...
    const draw = () => {
      ...
      if (e.shakeTimer > 0) {
        ctx.save();
        const sx = (Math.random() - 0.5) * e.shakeTimer * 2;
        const sy = (Math.random() - 0.5) * e.shakeTimer * 2;
        ctx.translate(sx, sy);
        e.shakeTimer--;
      }
      ...
      ctx.fillStyle = '#06060a'; ctx.fillRect(e.cameraX - 2500, GROUND_Y, CANVAS_WIDTH + 5000, 300);
      ctx.strokeStyle = '#00f3ff11'; ctx.lineWidth = 5;
      for (let gx = Math.floor((e.cameraX - 2500) / 120) * 120; gx < e.cameraX + 5500; gx += 120) { ctx.beginPath(); ctx.moveTo(gx, GROUND_Y); ctx.lineTo(gx, GROUND_Y + 300); ctx.stroke(); }
      ...
      e.enemies.forEach(en => { 
        if(!en.isDead) { 
          if (en.type === 'gigantic_titan') {
            ...
            ctx.restore();
          } else if (['titan', 'dragon', 'omnidroid', 'tank'].includes(en.type)) { drawBoss(ctx, en, e.frame); } 
